...
## Ported and linked for Sendspin

| Component | Where | Licence | Copyright |
|---|---|---|---|
| Sendspin time filter (2D Kalman filter, a Go port of `aiosendspin`'s `SendspinTimeFilter`, itself a port of the ESPHome reference) | `device/internal/sendspin/timefilter.go` | Apache-2.0 | Open Home Foundation / Sendspin contributors |
| `github.com/flynn/noise` (Noise Protocol Framework) | linked into the device binary | BSD-3-Clause | Prime Directive, Inc. |
| `github.com/mewkiz/flac` and `github.com/mewkiz/pkg` (pure-Go FLAC decoder) | linked into the device binary | Unlicense (public domain) | Robin Eklind |
| `github.com/icza/bitio` | linked into the device binary | Apache-2.0 | Andras Belicza |

`device/internal/sendspin/testdata/timefilter_golden.json` was generated by
running `aiosendspin` 9.1.1's implementation on a fixed input, so the Go port
is checked against the reference rather than against our reading of it.
